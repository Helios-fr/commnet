...
func Test_CreateUser() bool {
	...
	t1 := CreateUser("testuser", []byte("testpublickey"), []byte("testprivate"))
	ResetDB()
	return t1
}
...
func Test_UpdateUser() bool {
	...
	// Create the user
	CreateUser("testuser", []byte("testpublickey"), []byte("testprivate"))

	// Update the public key for the user
	t1 := UpdateUser("testuser", []byte("newpublickey"), []byte("newprivate"))
	...
	t2 := string(publicKey) == "newpublickey"
...
func Test_GetUser() bool {
	...
	t1 := string(publicKey) == "testpublickey"
...
func Test_ValidateUser() bool {
	...
	privateKey := []byte("testprivate")

	// Create the user
	CreateUser(username, publicKey, privateKey)

	// Check if the user is validated
	t1 := ValidateUser(username, publicKey)

	publicKey = []byte("invalidpublickey")
