...
func CreateUser(username string, publicKey []byte, privateKey []byte) bool {
	...
	if pub, priv := GetUser(username); len(pub) != 0 && len(priv) != 0 {
		return false
	}
	...
	// Write the new user to the csv file
	err = writer.Write([]string{username, string(publicKey), string(privateKey)})
	if err != nil {
